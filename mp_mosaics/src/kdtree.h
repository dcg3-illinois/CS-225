...
class KDTree
{
  ...
  private:
     ...
     */
     //will take a pivotIndex and place is so that all elements smaller are on the left
    //and all elements larger are on the right in a given dimension
    int partition(std::vector<Point<Dim>> &points, int left, int right, int dimension);

    //will take a list and place the median in the middle, with everything smaller in
    //one dimension on the left and greater in one dimension on the right
    Point<Dim> select(std::vector<Point<Dim>> & points, int left, int right, int medIndex, int dimension);


    //returns a KDTreeNode with all of the work done
    KDTreeNode* nextRoot(std::vector<Point<Dim>> & points, int left, int right, int medIndex, int dimension);

    KDTreeNode* copy(KDTreeNode* copyRoot);

    void delee(KDTreeNode* subroot);

    Point<Dim> findN(KDTreeNode* subroot, const Point<Dim>& query, int dimension) const;

    double distance(Point<Dim> p1, Point<Dim> p2) const;
};
