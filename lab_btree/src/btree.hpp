...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    //get the index where this would be inserted
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    //insert pointer into parent
    parent->children.insert(child_itr, new_right);
    //insert middle key of child into parent
    parent->elements.insert(elem_itr, child->elements.at(mid_elem_idx));
    //ensure child_itr is where the new right is
    child_itr = parent->children.begin() + child_idx + 1;
    //insert all of the good stuff into new_right
    (*child_itr)->elements.assign((mid_elem_itr + 1), child->elements.end());
    (*child_itr)->children.assign(mid_child_itr, child->children.end());
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    //case where it's larger than all the elements
    if(first_larger_idx >= subroot->elements.size()) {
        insert(subroot->children.at(first_larger_idx), pair);
        if(subroot->children.at(first_larger_idx)->elements.size() == size_t(order)) {
